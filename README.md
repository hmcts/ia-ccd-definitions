...
## CCD Definition Upload

The project includes scripts for uploading CCD definitions to different environments.

### Using the Unified Upload Script

```bash
# Upload to local dev environment
yarn upload-ccd -e dev

# Upload to mirrord environment (automatically uses your username)
yarn upload-ccd -e mirrord

# Upload to preview environment with a specific PR number
yarn upload-ccd -e preview -p 1234

# Test the upload configuration without actually uploading
yarn upload-ccd -e preview -p 1234 -d

# Show help
yarn upload-ccd -h
```

### Command Line Options for upload-ccd

| Option | Description | Default |
|--------|-------------|---------|
| `-e, --env` | Environment (dev, mirrord, preview) | `dev` |
| `-p, --pr` | PR number for preview environment (required when `-e preview` is specified) | - |
| `-d, --dry-run` | Show what would be done without actually uploading | `false` |
| `-h, --help` | Show help message | - |

### Direct Upload Scripts

For convenience, the following npm scripts are available for direct uploads:
...
# Upload to preview environment with a specific PR number
# Method: Set PR_NUMBER environment variable
PR_NUMBER=1234 yarn upload-preview
```

### URL Construction for Uploads

The upload script dynamically constructs URLs based on the environment and parameters:

- **Dev**: `http://localhost:4451`
