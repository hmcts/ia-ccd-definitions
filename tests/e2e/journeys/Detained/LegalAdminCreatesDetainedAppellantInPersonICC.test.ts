...
const feeRemission: string = ["Yes"].includes(process.env.FEE_REMISSION)
  ? "Yes"
  : "No";
const detentionLocation: string = [
  "immigrationRemovalCentre",
  "prison",
  "other"
].includes(process.env.DETENTION_LOCATION)
  ? process.env.DETENTION_LOCATION
  : "Prison";
...
const typeOfAppeal: string = [
  ...
  "protection"
].includes(process.env.APPEAL_TYPE)
...
test.describe(
  "Legal Admin creates Detained Appellant in Person " +
    typeOfAppeal +
    (isRehydrated ? " Rehydrated " : " Paper ") +
    "ICC Case " +
    (inTime ? "In Time" : "Out of Time"),
  { tag: "@LegalAdminCreatesDetainedAppellantInPersonICC" },
  () => {
      ...
      async ({ page }) => {
        ...
        if (isRehydrated) {
          ...
          await validationHelper.validateLabelDisplayed(
            ...
            imageLocators.rehydrated.detained.appellantInPersonManual.name
          );
        } else {
          await validationHelper.validateLabelDisplayed(
            imageLocators.detained.appellantInPersonManual.locator,
            imageLocators.detained.appellantInPersonManual.name
          );
        }
        ...
        await validationHelper.validateCaseFlagExists(
          "Detained individual",
          "Active"
        );
        ...
        isRehydrated
          ? await validationHelper.validateLabelDisplayed(
              imageLocators.rehydrated.detained.appellantInPersonManualS94b
                .locator,
              imageLocators.rehydrated.detained.appellantInPersonManualS94b.name
            )
          : await validationHelper.validateLabelDisplayed(
              imageLocators.detained.appellantInPersonManualS94b.locator,
    ...
    test("Home Office Officer (respondent) review appeal and upload Home Office bundle", async ({
      page
    }) => {
    ...
    test("Legal Officer directs appellant/Legal Rep to build case", async ({
      page
    }) => {
      await idamPage.login(legalOfficerCredentials);
      await pageHelper.getCase(caseId);
    ...
    test("Legal Officer creates Respondent Review Direction", async ({
      page
    }) => {
      await idamPage.login(legalOfficerCredentials);
      await pageHelper.getCase(caseId);
    ...
    test("Home Office Officer (respondent) responds to appeal response from Appellant/Legal Rep", async ({
      page
    }) => {
      await idamPage.login(homeOfficeOfficerCredentials);
      await page.goto(envUrl + "/cases/case-details/" + caseId);
    ...
    test("Legal Officer Force case - hearing reqs, thus bypassing Appellant/Legal Rep needing to review the HO decision", async ({
      page
    }) => {
      await idamPage.login(legalOfficerCredentials);
      await pageHelper.getCase(caseId);
    ...
    test("Appellant/legal rep submit hearing requirements", async ({
      page
    }) => {
      await idamPage.login(legalOfficerAdminCredentials);
    ...
    test("Listing Officer to create the case summary, generate hearing bundle and start decision and reasons", async ({
      page
    }) => {
      ...
      await expect(
        page.locator(" #progress_caseOfficer_preHearing")
      ).toBeVisible();
      await new StartDecisionAndReasons(page).submit("Yes", "Yes");
      await linkHelper.signOut.click();
    });

    test("Judge to Prepare and Complete decision and reasons", async ({
      page
    }) => {
