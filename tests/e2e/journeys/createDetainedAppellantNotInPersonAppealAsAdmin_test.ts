...
Feature('Legally Represented - Manual Detained Appeal - Legal Admin @LegalAdminDetainedAppellantNotInPerson');
...
// @ts-ignore
Scenario('Create Detained Appeal - Appellant Not In Person as Legal Admin - ' + (inTime ? 'In Time' : 'Out of Time'),   async ({I, loginPage, createCasePage, createAppeal, draftAppeal}) => {
...
// @ts-ignore
Scenario('Admin removes detained status for Legally Represented - Manual',   async ({I, loginPage, removeDetainedStatus}) => {
    await loginPage.signIn(legalAdmin);
