...
let legalRepDetained: LegalRepDetained;
...
test.describe('Legal Representative creates Detained Represented ' + typeOfAppeal + (inTime ? ' In Time, ' : ' Out of Time, ')  + 'DRAFT Appeal.', { tag: '@LrDetainedApi' }, () => {
    ...
    test.beforeAll(async ({  }) => {
        ...
        legalRepDetained = new LegalRepDetained();
    ...
    test('Create detained'  + ' DRAFT Appeal',   async ({ page }) => {
        ...
        eventData = await legalRepDetained.generateDraftData();
        ...
        // validate the data saved successfully
    ...
    test('Submit: Request Respondent Evidence event', async ({}) => {
        ...
        delete caseData['feeWithHearing'];
