...
const BASE_CASE_API_DIR = process.env.IA_CASE_API_DIR !== undefined ? process.env.IA_CASE_API_DIR : '../ia-case-api/';
...
const existingCaseFields = JSON.parse(fs.readFileSync('definitions/appeal/json/CaseField.json', 'utf8'))
    .map(caseField => caseField.ID);

// extract non existing fields from code
const nonExistingCaseFields = fs.readFileSync(BASE_CASE_API_DIR + FIELD_DEFINITIONS, 'utf8')
    .match(/\"\w+\"/g)
    .map(s => s.slice(1, -1))
...
const pathsToCheck = [];
walkSync(BASE_CASE_API_DIR, {globs: ['src/main/java/**/*.java'], directories: false})
    .filter(file => file !== FIELD_DEFINITIONS)
    .forEach(path => {
        let content = fs.readFileSync(BASE_CASE_API_DIR + path, 'utf8');
