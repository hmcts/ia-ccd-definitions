...
const fileName = path.join(__dirname, '../definitions/appeal/json/CaseEventToFields.json');
const fileNameBail = path.join(__dirname, '../definitions/bail/json/CaseEventToFields.json');

try {
  const content = fs.readFileSync(fileName, 'utf8');
  const contentBail = fs.readFileSync(fileNameBail, 'utf8');
  checkContent(content);
  checkContent(contentBail);
} catch (err) {
  console.error(err);
  process.exit(1);
}
